...
int handle_request(struct http_request *request, struct http_response *response) {
    ...
    if (path_len > MAX_PATH_LEN) {
        char *body = malloc(75);
        add_static_status_to_response(response, get_status_code_string(400));
        response->body_size = (size_t) sprintf(body, "<h1>400 Bad Request</h1><p>Path too long. Max %zu characters.</p>", MAX_PATH_LEN);
    ...
    if (filedata->is_dir) {
        ...
        response->body_size = (size_t) sprintf(body, "<h1>501 Not Implemented</h1><p>The file %s is a directory. Contents listing coming soon.</p>", path);
        add_static_body_to_response(response, body);
        add_header_to_response(response, "Content-Type", "text/html");
        return 0;
    }
    ...
    if (request->query_string != NULL && strcmp(request->query_string, "metadata") == 0) {
        ...
        response->body_size = (size_t) sprintf(body, "<h1>File metadata: %s</h1><ul><li>Extension: %s</li><li>Size: %zub</li></ul>", path, filedata->file_ext, filedata->buf_size);
        add_static_body_to_response(response, body);
        add_header_to_response(response, "Content-Type", "text/html");
    } else {
